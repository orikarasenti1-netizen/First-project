...
function renderToday() {
  ...
  state.habits.forEach(h => {
    ...
    li.innerHTML = `
      ...
      ${s > 0 ? `<span class="habit-streak" aria-label="${s} day streak"><span class="streak-gem" aria-hidden="true">◆</span>${s}d</span>` : ''}
      <button class="habit-del" aria-label="Delete ${esc(h.name)}" tabindex="-1">
        <svg width="11" height="11" viewBox="0 0 11 11" fill="none" aria-hidden="true">
          <line x1="1.5" y1="1.5" x2="9.5" y2="9.5" stroke="currentColor" stroke-width="1.75" stroke-linecap="round"/>
          <line x1="9.5" y1="1.5" x2="1.5" y2="9.5" stroke="currentColor" stroke-width="1.75" stroke-linecap="round"/>
        </svg>
      </button>
    `;
    function onToggle(e) {
...
function renderHeatmap() {
  ...
  dates.forEach(date => {
    ...
    if (!future) cell.setAttribute('aria-label', `${shortDate(date)}: ${count} of ${maxH}`);
...
function toggle(id, li) {
  ...
  if (isDone(h, t)) {
    ...
    burst(li.querySelector('.habit-check'), h.color);
  }
  save(); renderToday(); renderStats(); renderHeatmap();
}
...
function burst(origin, color) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    p.style.cssText = `left:${cx-2.5}px;top:${cy-2.5}px;background:${color};--tx:${Math.cos(angle)*dist}px;--ty:${Math.sin(angle)*dist}px`;

...
function renderHeatmap() {
  ...
  // Start on Monday of 15 weeks ago
  const dow   = td.getDay(); // 0=Sun
  const toMon = (dow === 0) ? 6 : dow - 1; // days since last Mon
  const thisMon = new Date(td);
  thisMon.setDate(thisMon.getDate() - toMon);

  const startDate = new Date(thisMon);
  startDate.setDate(startDate.getDate() - 14 * 7); // 15 weeks back

  const endDate = new Date(thisMon);
  endDate.setDate(endDate.getDate() + 6); // Sunday of current week

  // Build date array Mon → Sun × 15 weeks
  const dates = [];
